...
; 0. create new row
; 1. Send Proofreading & Tuning
; 2. Down, Home, Timestamp somehow
; 	 copy duration
; 3. Right, Right, Logging
; 4. Many Rights depending on emoji columns
; 5. clean up code → 
;:*?:pfr!@::
	sleepTime := 25
	longSleepTime := 100

	; open cell context menu, create row
	Send {^i}
	Send {alt down}
	Sleep, %longSleepTime%
	Send I
	...
	Sleep, %longSleepTime%
	Send {R}{R}
	Send Proofreading & Tuning
	Send {Down}
	Send {Home}

	; create timestamp via google sheets hotkey
	Send {alt down}
	Send {ctrl down}
	Send {shift down}
	Send {;}
	...
	; copy duration → right, down, copy, up, paste, right
	; 🌟 every arrow key needs a sleep50 → escape for unknown reason
	Send {right}
	Sleep, %sleepTime%
	Send {escape}
	...
	; right four times → cursor now in correct logging cell
	Loop, 4 {
		Send {right}
		Sleep, %sleepTime%
		Send {escape}
